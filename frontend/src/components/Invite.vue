...
const inviteStudent = async () => {
  ...
  try {
    await apiClient.post(`/group/${groupId}/invite`,null, {
...
const getMembers = async () => {
  try {
      const response = await apiClient.get('/group/getMembers', {
...
const removeMember = async student => {
  try {
    const response = await apiClient.delete(`/group/${groupId}/removeMember`, {
...
<template>
  <form novalidate class="space-y-4 md:space-y-6" @submit.prevent="inviteStudent">
    ...
    <button type="submit" class="w-full text-gray-50 border-gray-700 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Пригласить</button>
  </form>
  ...
  <div class="bg-gray-100 rounded shadow overflow-y-auto" :key="groupId">
    <ul>
      ...
      >
        ...
        <button @click="removeMember(student)" class="px-3 py-1 text-white disabled:bg-slate-600 bg-gray-700 hover:bg-gray-800 rounded-lg text-sm relative z-10">{{ student.status === 'EXPECTED' ? 'Отозвать приглашение' : (student.status === 'DECLINED' ? 'Удалить запись' : 'Исключить из группы') }}</button>
      </li>
    </ul>
  </div>
</template>
