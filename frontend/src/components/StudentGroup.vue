...
function seekToTime(seconds) {
  if (store.state.videoPlayer.value) {
    store.state.videoPlayer.currentTime = seconds;
  }
}
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div class="bg-gray-100 rounded shadow flex flex-col p-2 gap-4 h-[27.2rem]">
      <SelectVideo :videos="videos" class="flex flex-col h-full"></SelectVideo>
    </div>
