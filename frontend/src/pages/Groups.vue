...
const createGroup = () => {
  ...
  try {
    const response = apiClient.post('/group/create', {
    ...
    <div class="bg-gray-100 rounded shadow p-2 flex flex-col gap-4">
      <div v-if="typeUser === 'ROLE_TEACHER'">
        ...
        <form novalidate class="space-y-4 md:space-y-6" @submit.prevent="createGroup">
          <div>
            <label for="groupName" class="block mb-2 text-sm font-medium text-gray-900 ">Название</label>
          ...
          <button type="submit" class="w-full text-gray-50 border-gray-700 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Создать</button>
        </form>
      </div>

      <div v-if="typeUser === 'ROLE_STUDENT'" class="">
        <h3 class="block text-lg font-medium text-gray-900 ">Приглашения</h3>
        <InvitingStudents class="h-full"></InvitingStudents>
      </div>
